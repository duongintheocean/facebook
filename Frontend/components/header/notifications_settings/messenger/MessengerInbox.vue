<template>
  ...
  >
    ...
    <messenger-inbox-top
      :messenger-value-one="messengerValueOne"
      :inbox-one-focus="toggle.inboxOneFocus"
      @handle-close-show-inbox="handleCloseShowInbox"
    />

    <!-- Phần trên của inboxTwo -->
    <messenger-inbox-top
      :messenger-value-two="messengerValueTwo"
      :inbox-one-focus="toggle.inboxOneFocus"
      @handle-close-show-inbox-two="handleCloseShowInboxTwo"
    />
    ...
    <div class="flex h-[410px] flex-col justify-between">
      ...
      <div
        ...
        class="flex flex-col justify-start min-h-[150px] w-full overflow-y-scroll"
      >
          ...
          <div v-if="messagesList?.length > 0">
            <div>
              ...
              >
                ...
                <div
                  v-if="
                    message.userId == messengerValueOne?.toUserId &&
                ...
                >
                  ...
                  <div class="flex flex-col items-start">
                    ...
                    <div
                      v-if="message.messageReplyContent && messengerValueOne"
                ...
                <div
                  v-if="
                    message.userId == messengerValueTwo?.toUserId &&
                    message.userId !== user.user_id
                  "
                  class="flex items-end mt-2 mb-2 cursor-pointer"
                >
                  ...
                  <div class="flex flex-col items-start">
                    <!-- Phần trên của tin nhắn phản hồi -->
                    <div
                      v-if="message.messageReplyContent && messengerValueTwo"
                      class="flex items-center text-[12px] text-gray-500 ml-3 mt-2 mb-2"
                    >
                      <div class="w-[15px] h-[15px]">
                        <img
                      ...
                      <div class="ml-1">
                        <span>{{ messengerValueTwo?.toUserName }}</span> đã trả
                        lời bạn
                      </div>
                    </div>
        ...
        >
          ...
          <div v-if="messagesList?.length > 0">
            <div>
              ...
              >
                ...
                <div
                  v-if="
                    messageTwo.userId == messengerValueTwo?.toUserId &&
                    messageTwo.userId !== user.user_id
                  "
                  class="flex items-end mt-2 mb-2"
                >
                  <div
                    style="border: 1px rgb(206, 206, 206) solid"
                    class="w-7 h-7 rounded-full overflow-hidden ml-2 mr-2"
                  ...
                  <!-- Tin nhắn thường cộng phản hồi của người khád -->
                  <div class="flex flex-col items-start">
                    <!-- Phần trên của tin nhắn phản hồi -->

                    <!-- InboxTwo -->
                    <div
                      v-if="messageTwo.messageReplyContent && messengerValueTwo"
                      class="flex items-center text-[12px] text-gray-500 ml-3 mt-2 mb-2"
                    >
                      <div class="w-[15px] h-[15px]">
                        <img
          ...
          <!-- InboxOne -->
          <div
            v-if="messengerValueOne && messengerValueOne.unread > 0"
...
export default {
  ...
  computed: {
    // Thay đổi chiều rộng của input khi nhập dữ liệu
    calculateCols() {
      return this.value.inputData || this.value.previewImg ? 25 : 10
    },
  ...
  methods: {
    ...
    async handleRenderConversation() {
      try {
        ...
        if (this.messengerValueOne) {
          if (this.messengerValueOne.conversationId) {
            response = await messengerRepository.renderByConversationId({
              accountHasLoginId: this.user.user_id,
              conversationId: this.messengerValueOne.conversationId,
            })
            console.log('111111', response.data)
          } else {
            response = await messengerRepository.renderByToUserId({
              accountHasLoginId: this.user.user_id,
        ...
        if (this.messengerValueTwo) {
          if (this.messengerValueTwo.conversationId) {
            response = await messengerRepository.renderByConversationId({
              accountHasLoginId: this.user.user_id,
              conversationId: this.messengerValueTwo.conversationId,
            })
          } else {
            response = await messengerRepository.renderByToUserId({
              accountHasLoginId: this.user.user_id,
    ...
    // Đóng inboxOne
    handleCloseShowInbox(value) {
      this.$emit('show-inbox-one', value)
    },

    // Đóng inboxTwo
    handleCloseShowInboxTwo(value) {
      this.$emit('show-inbox-two', value)
    },
